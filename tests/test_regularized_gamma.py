...
def test_alpha_shrinks_coefficients():
    ...
    model_unreg = gamma_glm(alpha=0.0)
    model_reg = gamma_glm(alpha=1.0)
    model_unreg.fit(X, y)
    model_reg.fit(X, y)
...
def test_alpha_zero_matches_unregularized():
    X, y, _ = make_dataset(10000)
    model_default = gamma_glm()
    model_explicit_zero = gamma_glm(alpha=0.0)
...
def test_intercept_not_regularized():
    X, y, _ = make_dataset(10000)
    model_unreg = gamma_glm(alpha=0.0)
    model_reg = gamma_glm(alpha=10.0)
    model_unreg.fit(X, y)
    model_reg.fit(X, y)
